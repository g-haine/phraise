...
PHRAISE currently pins **BibReview v1.6.12** for reproducible maintenance and
...
The Conda environment contains Python 3.12 and **BibReview v1.6.12**.
...
With OpenAlex enabled in `bibreview.yml`, BibReview v1.6.12 uses it for three
...
BibReview v1.6.12 provides two complementary workflows for existing canonical
records.
...
BibReview v1.6.12 also allows scalar-field backfill from partial provider records:
...
For `abstract`, BibReview v1.6.12 treats the historical placeholder
...
Optional publisher enrichment is failure-tolerant in v1.6.12. If Elsevier,
...
BibReview v1.6.12 can compare the existing canonical bibliography with current
...
BibReview v1.6.12 provides a derived read-only review that applies the current
...
BibReview v1.6.12, page-range proposals are normalized to BibTeX-style double
...
unchanged. BibReview v1.6.12 also reports accepted/custom resolutions that already
...
- exact BibReview v1.6.12 installation;
