#!/usr/bin/env bash
set -euo pipefail
printf "Warning: getData.sh is deprecated; use python getData.py\n" >&2
...
while IFS= read -r doi; do
  ...
  if [[ "$status" == "ok" ]]; then
    ...
    authors=$(read_json "$response" '.message.author')
    type=$(read_json "$response" '.message.type // ""')
    abstract=$(read_json "$response" '.message.abstract // ""')
    journal=$(read_json "$response" '.message["container-title"] // [""] | .[0]')
    year=$(read_json "$response" '.message["published-print"]["date-parts"][0][0] // ""')
    volume=$(read_json "$response" '.message.volume // ""')
    issue=$(read_json "$response" '.message.issue // ""')
    event=""
    isbn=$(read_json "$response" '.message["isbn-type"][0]["value"] // ""')
    pages=$(read_json "$response" '.message.page // "" | gsub("-"; "--")')
    publisher=$(read_json "$response" '.message.publisher // ""')
    keywords=$(read_json "$response" '.message.subject // [] | join(", ")')
    dateY=$(read_json "$response" '.message.created["date-parts"][0][0] // ""')
    dateM=$(read_json "$response" '.message.created["date-parts"][0][1] // ""')
    dateD=$(read_json "$response" '.message.created["date-parts"][0][2] // ""')
    ...
    log " DOI: $doi is not available yet."
  fi
...
log $(date -Iseconds)" Data get successfully!"
exit 0
